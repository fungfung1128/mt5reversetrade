...
class MT5TradeGenerator(QMainWindow):
    def __init__(self):
        ...
        # 定義產品名稱映射
        self.mt5_symbol = "XAUUSD.ECN"  # MT5 使用的產品名稱
        self.google_symbol = "xauusd"      # Google Sheets 使用的產品名稱
        self.internal_symbol = "XAUUSD"    # 內部統一使用的鍵名
        ...
        # 初始化數據
        self.current_positions = {}  # 儲存當前持倉 {product: lots}
        self.google_positions = {}  # 儲存 Google Sheets 持倉 {product: lots}
    ...
    def connect_to_mt5_and_google_sheets(self):
        try:
            ...
            creds = ServiceAccountCredentials.from_json_keyfile_name(
                r'C:\Users\fungb\OneDrive\桌面\impactful-name-455509-b6-b07e866843f7.json',
                scope
            )
    ...
    def update_mt5_positions(self):
        positions = mt5.positions_get(symbol=self.mt5_symbol)  # 使用 MT5 的產品名稱
    ...
    def execute_trades(self):
        ...
        for product, google_lot in self.google_positions.items():
            ...
            lots = abs(difference)
            symbol = self.mt5_symbol  # 使用 MT5 的產品名稱
